...
class _Reducer(_Transformer):
    def __init__(self, name: str, n_components: int = 2):
        super().__init__(self.__class__.__name__, n_components)
        self.suffix = f" ({name})"
    ...
    def call(self, df: DataFrame) -> DataFrame:
...
class PCA(_Reducer):
    def __init__(self, name: str = "PCA", n_components: int = 2):
        super().__init__(name, n_components)

    def _set_model(self):
        self._model = PCAsk(n_components=self.n_components)
...
class TSNE(_Reducer):
    def __init__(self, name: str = "TSNE", n_components: int = 2, perplexity: float = 30):
        super().__init__(name, n_components)
        self.perplexity = perplexity

    def _set_model(self):
        self._model = TSNEsk(n_components=self.n_components, perplexity=self.perplexity)
...
class UMAP(_Reducer):
    def __init__(self, name: str = "UMAP", n_components: int = 2):
        super().__init__(name, n_components)
